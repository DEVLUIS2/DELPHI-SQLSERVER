object TGraficoTop10Produto: TTGraficoTop10Produto
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  Caption = 'Gr'#225'fico Top 10 Produtos Mais Comprados'
  ClientHeight = 466
  ClientWidth = 731
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  WindowState = wsMaximized
  OnKeyDown = FormKeyDown
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 431
    Width = 731
    Height = 35
    Align = alBottom
    TabOrder = 0
    ExplicitWidth = 733
    object LabPage: TLabel
      Left = 77
      Top = 1
      Width = 40
      Height = 33
      Align = alLeft
      Alignment = taCenter
      Caption = '    0/0    '
      Layout = tlCenter
      ExplicitHeight = 13
    end
    object btnNext: TBitBtn
      AlignWithMargins = True
      Left = 120
      Top = 4
      Width = 70
      Height = 27
      Align = alLeft
      Caption = '>'
      TabOrder = 0
      OnClick = btnNextClick
    end
    object btnPrev: TBitBtn
      AlignWithMargins = True
      Left = 4
      Top = 4
      Width = 70
      Height = 27
      Align = alLeft
      Caption = '<'
      TabOrder = 1
      OnClick = btnPrevClick
    end
    object btnFechar: TBitBtn
      AlignWithMargins = True
      Left = 627
      Top = 4
      Width = 100
      Height = 27
      Align = alRight
      Caption = 'Fechar [ESC]'
      Glyph.Data = {
        
        8000800080008000800080008000800080008000800080008000}
      NumGlyphs = 2
      TabOrder = 2
      OnClick = btnFecharClick
      ExplicitLeft = 629
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 731
    Height = 431
    Align = alClient
    TabOrder = 1
    ExplicitWidth = 733
    object DBChart1: TDBChart
      Left = 1
      Top = 1
      Width = 729
      Height = 429
      AllowPanning = pmNone
      MarginRight = 5
      Title.Font.Color = clBlack
      Title.Font.Height = -19
      Title.Font.Style = [fsBold]
      Title.Text.Strings = (
        'Top 10 Produtos Mais Comprados')
      BottomAxis.LabelsSize = 23
      BottomAxis.TickLength = 0
      BottomAxis.Title.Caption = 'Produtos'
      BottomAxis.Title.Font.Height = -16
      BottomAxis.Title.Font.Style = [fsBold]
      BottomAxis.Title.Font.InterCharSize = 2
      LeftAxis.LabelsSize = 28
      LeftAxis.Title.Caption = 'Quantidade Comprada'
      LeftAxis.Title.Font.Height = -15
      LeftAxis.Title.Font.Style = [fsBold]
      LeftAxis.Title.Font.InterCharSize = 1
      Legend.Alignment = laBottom
      Legend.Gradient.EndColor = clSilver
      Legend.Gradient.MidColor = clSilver
      Legend.Gradient.Visible = True
      Legend.Symbol.Gradient.StartColor = clSilver
      Legend.Title.Gradient.EndColor = clSilver
      Legend.Title.Gradient.MidColor = clSilver
      Legend.Title.Gradient.StartColor = clSilver
      Legend.Title.Text.Strings = (
        'LEGENDA')
      Legend.Title.TextAlignment = taCenter
      Pages.AutoScale = True
      Pages.MaxPointsPerPage = 3
      OnPageChange = DBChart1PageChange
      Align = alClient
      BevelOuter = bvNone
      TabOrder = 0
      ExplicitWidth = 731
      object Series1: TBarSeries
        ColorEachPoint = True
        Marks.Arrow.Visible = True
        Marks.Callout.Brush.Color = clBlack
        Marks.Callout.Arrow.Visible = True
        Marks.Style = smsValue
        Marks.Visible = True
        DataSource = qGraficoProduto
        Title = 'Top10Produtos'
        XLabelsSource = 'DESCRICAO_PROD'
        BarWidthPercent = 45
        Gradient.Direction = gdTopBottom
        OffsetPercent = -5
        XValues.Name = 'X'
        XValues.Order = loAscending
        YValues.Name = 'Bar'
        YValues.Order = loNone
        YValues.ValueSource = 'Produtos Comprados'
      end
    end
  end
  object qGraficoProduto: TADOQuery
    Connection = TDMPrincipal.Connection
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'SELECT TOP 10 SUM(CI.QUANTIDADE_ITENS) AS '#39'Produtos Comprados'#39' ,' +
        'P.COD_PROD,P.DESCRICAO_PROD,F.NOME_FORNECEDOR'
      'FROM FORNECEDORES F'
      'INNER JOIN PRODUTO P ON P.COD_FORNECEDOR = F.COD_FORNECEDOR'
      'INNER JOIN COMPRA_ITENS CI ON CI.COD_PROD = P.COD_PROD'
      'INNER JOIN COMPRA C ON C.COD_COMPRA = CI.COD_COMPRA'
      'GROUP BY P.DESCRICAO_PROD,P.COD_PROD,F.NOME_FORNECEDOR'
      'ORDER BY SUM(CI.QUANTIDADE_ITENS) DESC')
    Left = 32
    Top = 379
  end
end
